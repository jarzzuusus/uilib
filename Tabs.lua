    ...
    function TabFuncs:AddTab(TabConfig)
        ...
        function SectionFuncs:AddSection(SectionConfig)
            ...
            HeaderBtn.MouseButton1Click:Connect(function()
                ...
                if isOpen then
                    task.spawn(function()
                        local stableContentH = ContentFrame.AbsoluteSize.Y
                        local prevH = -1
                        local stableCount = 0
                
                        -- Polling sampai tinggi content stabil (gak berubah) selama 2 frame berturut-turut
                        for i = 1, 10 do
                            task.wait()
                            local h = ContentFrame.AbsoluteSize.Y
                            if h == prevH and h > 0 then
                                stableCount += 1
                        ...
                        if stableContentH <= 0 then
                            stableContentH = getContentHeight()
                        end
                
                        TweenService:Create(SectionFrame, TweenInfo.new(0.35, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
                            Size = UDim2.new(1, 0, 0, HEADER_H + stableContentH)
                        }):Play()
                    end)
