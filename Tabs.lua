...
return function(
    ...
    function TabFuncs:AddTab(TabConfig)
        ...
        function SectionFuncs:AddSection(SectionConfig)
            ...
            if SectionConfig.Open ~= nil then
                startOpen = SectionConfig.Open
            else
                startOpen = true
            end
            ...
            local function getContentHeight()
                ...
                for _, child in ipairs(ContentFrame:GetChildren()) do
                    if child:IsA("GuiObject") and child.Name ~= "UIListLayout" and child.Name ~= "UIPadding" then
                        local knownH = child:GetAttribute("ItemHeight")
                        h = h + (knownH or child.AbsoluteSize.Y) + 4
                    end
                end
                h = h + 4 + 8
                return h
            ...
            local function MakeItemWrapper(height)
                ...
                Row:SetAttribute("ItemHeight", height)
                return Row
            end
            ...
            local function applyInitialState()
                if isOpen then
                    SectionFrame.Size = UDim2.new(1, 0, 0, HEADER_H + getContentHeight())
                else
            ...
            function ItemFuncs:AddParagraph(Cfg)
                ...
                -- Hitung lebar yang bakal dipakai text (ContentFrame width - padding kiri kanan ContentFrame - padding BG)
                local availWidth = ContentFrame.AbsoluteSize.X
                if availWidth <= 0 then
                    availWidth = SectionScroll.AbsoluteSize.X - 24 -- fallback estimasi
                end
                availWidth = availWidth - 12 - 12 - 24 -- padding ContentFrame (6+6) + padding BG (12+12)
            
                local totalH = 10 + 10 -- padding top+bottom BG
                local titleH, contentH = 0, 0
                ...
                if ptitle ~= "" then
                    ...
                    local bounds = TextService:GetTextSize(ptitle, 13, Enum.Font.GothamBold, Vector2.new(availWidth, math.huge))
                    titleH = bounds.Y
                end
                ...
                if pcontent ~= "" then
                    ...
                    local bounds = TextService:GetTextSize(pcontent, 11, Enum.Font.Gotham, Vector2.new(availWidth, math.huge))
                    contentH = bounds.Y
                end
            
                totalH = totalH + titleH + contentH
                if ptitle ~= "" and pcontent ~= "" then
                    totalH = totalH + 4 -- spacing innerList
                end
            
                Row:SetAttribute("ItemHeight", totalH)
